...
class PredictRequest(BaseModel):
    ...
    CompetitionDistance: float = Field(
        ..., ge=0, description="Distance to nearest competitor."
    )
    Promo2: int = Field(
        ..., ge=0, le=1, description="Store participates in promo2 (0/1)."
    )
    ...
    CompetitionDistance_missing: int = Field(
        ..., ge=0, le=1, description="CompetitionDistance was missing (0/1)."
    )
    competition_age_months: float = Field(
        ..., ge=0, description="Months since competition opened."
    )
    has_competition: int = Field(
        ..., ge=0, le=1, description="Store has a competitor (0/1)."
    )
    ...
    promo2_active: int = Field(
        ..., ge=0, le=1, description="Promo2 active this month (0/1)."
    )
    promo2_age_months: int = Field(
        ..., ge=0, description="Months since promo2 started."
    )
    store_mean_sales: float = Field(
        ..., ge=0, description="Store historical mean sales."
    )
    store_mean_sales_dow: float = Field(
        ..., ge=0, description="Store mean sales for this day-of-week."
    )
    ...
    days_to_holiday: int = Field(
        ..., ge=0, description="Days until next state holiday."
    )
    days_since_holiday: int = Field(
        ..., ge=0, description="Days since last state holiday."
    )
    ...
    PromoInterval_Feb_May_Aug_Nov: int = Field(
        ..., ge=0, le=1, description="PromoInterval Feb/May/Aug/Nov (0/1)."
    )
    PromoInterval_Jan_Apr_Jul_Oct: int = Field(
        ..., ge=0, le=1, description="PromoInterval Jan/Apr/Jul/Oct (0/1)."
    )
    PromoInterval_Mar_Jun_Sept_Dec: int = Field(
        ..., ge=0, le=1, description="PromoInterval Mar/Jun/Sept/Dec (0/1)."
    )
    PromoInterval_None: int = Field(
        ..., ge=0, le=1, description="No promo interval (0/1)."
    )
